...
const addRangeQueryToObject = (object, key, minValue, maxValue) => {
    console.log(!isNaN(parseFloat(minValue)));
    if (!!minValue && !isNaN(parseFloat(minValue)))
        object[`min-${key}`] = minValue;
    if (!!maxValue && !isNaN(parseFloat(maxValue)))
        object[`max-${key}`] = maxValue;
    console.log(object);
};

function AdvancedSearch({ dispatchSearchParamsData }) {
    const [isBasicSearchOpen, setIsBasicSearchOpen] = useState(false);
    const [areAdvancedFeaturesOpen, setAreAdvancedFeaturesOpen] =
        useState(false);
    const [extraFeaturesBtnTxt, setExtraFeaturesBtnTxt] =
        useState("חיפוש מתקדם");
    ...
    useEffect(() => {
        if (chosenFeaturesCounter === 0)
            return setExtraFeaturesBtnTxt("חיפוש מתקדם");
    ...
    useEffect(() => {
        ...
        if (chosenTypesLen === 1)
            return (typeRef.current.value = chosenTypes[0]);
        typeRef.current.value = `${chosenTypesLen} סוגי נכסים`;
    }, [chosenApartmentTypes, chosenHouseTypes, chosenExtraTypes]);

    useEffect(() => {
        if (
            !minRoomsNumRef.current ||
            !maxRoomsNumRef.current ||
            !roomsNumRef.current
        )
            return;

        const isMinRoomsSpecified = !(
            minRoomsVal === "הכל" || minRoomsVal === 0
        );
        const isMaxRoomsSpecified = !(
            maxRoomsVal === "הכל" || maxRoomsVal === 0
        );
    ...
    useEffect(() => {
        ...
        if (
            (!!minFloorsVal && minFloorsVal !== "הכל") ||
            (!!maxFloorsVal && maxFloorsVal !== "הכל")
        )
            counter++;
        if (
            (!!minSizeVal && minSizeVal !== "הכל") ||
            (!!maxSizeVal && maxSizeVal !== "הכל")
        )
            counter++;
    ...
    const searchOnSubmit = async (e) => {
        ...
        if (chosenLocation) {
            let response;
            try {
                response = await axios.get("/data/citiesArray", {
                    headers: {
                        "Content-Type": "application/json",
                        Accept: "application/json",
                    },
            ...
            if (!!response) {
                if (response.data.includes(chosenLocation))
                    newQueryParams.town = chosenLocation;
        ...
        for (let i = 0; i < chosenApartmentTypes.length; i++)
            types.push(
                apartmentTypesEn[
                    apartmentTypes.indexOf(chosenApartmentTypes[i])
                ]
            );
    ...
    return (
        <div className="advanced-search" onClick={closeOpenDropdown}>
            <h3>
                איזה נכס ל<span className="link-span">מכירה</span> תרצו לחפש?
            </h3>
            ...
            {isBasicSearchOpen && (
                <form
                    ...
                    className={
                        areAdvancedFeaturesOpen ? "" : "basic-search-form"
                    }
                >
