...
function FilesModal({ files, setIsDisplayFilesModalOpen }) {
    ...
    const [enlargeBtnClassName, setEnlargeBtnClassName] = useState(defaultEnlargeBtnClassName);
    ...
    return (
        <div className="modal-wrapper files-modal-wrapper" onClick={() => { setIsDisplayFilesModalOpen(false); }}>
            <div className="modal-content" onClick={(e) => { e.stopPropagation(); }}>
                { !isImageEnlarged ?
                <>
                    <button onClick={() => { setIsDisplayFilesModalOpen(false); }}>
                    ...
                    <div
                        ...
                        onMouseEnter={() => { fileContainerOnEnterOrLeave("ENTER"); }}
                        onMouseLeave={() => { fileContainerOnEnterOrLeave("LEAVE"); }}
                    >
                        <div className={arrowClassName} onClick={() => { navigateFilesOnClick("PREV"); }}>&#x276C;</div>
                        <div className={arrowClassName} onClick={() => { navigateFilesOnClick("NEXT"); }}>&#x276D;</div>

                        { !files[currFileIndex].type.includes('video') &&
                            <button className={enlargeBtnClassName} onClick={() => { setIsImageEnlarged(true); }}>
                        ...
                        <span className="file-counter-message">{`תמונה ${currFileIndex + 1} מתוך ${files.length}`}</span>
                    </div>
                </>
                :
                <>
