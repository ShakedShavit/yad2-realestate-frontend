...
function SignupSecondPage(props) {
    ...
    const [startTelNumVal, setStartTelNumVal] = useState("050");
    ...
    const firstNameOnChange = (e) => {
        ...
        if (!lettersRegEx.test(firstName))
            return setFirstNameErrMsg(nonLettersFieldErrMsg);
    ...
    const lastNameOnChange = (e) => {
        ...
        if (!lettersRegEx.test(lastName))
            return setLastNameErrMsg(nonLettersFieldErrMsg);
    ...
    const telNumOnChange = (e) => {
        ...
        if (!telNumRegEx.test(telNum) || telNumLen !== 7)
            return setTelNumErrMsg("מספר פלאפון לא תקין");
    ...
    useEffect(() => {
        ...
        if (!!firstNameErrMsg)
            return setFirstNameClassName(invalidInputClassName);
    }, [firstNameErrMsg, isFirstNameValid]);
    useEffect(() => {
        if (isLastNameValid) return setLastNameClassName(validInputClassName);
        if (!!lastNameErrMsg)
            return setLastNameClassName(invalidInputClassName);
    ...
    return (
        <>
            <form
                ...
                className="login-form second-signup-form"
            >
                <div className="form-body">
                    ...
                    <input
                        ...
                        onBlur={firstNameOnChange}
                    ></input>
                    {firstNameErrMsg.length !== 0 && (
                        <span className="input-err-msg">{firstNameErrMsg}</span>
                    )}
                    ...
                    <input
                        ...
                        onBlur={lastNameOnChange}
                    ></input>
                    {lastNameErrMsg.length !== 0 && (
                        <span className="input-err-msg">{lastNameErrMsg}</span>
                    )}
                    ...
                    <div>
                        <input
                            ...
                            maxLength="7"
                            placeholder="טלפון"
                            className={telNameClassName}
                            onChange={telNumOnChange}
                            onBlur={telNumOnChange}
                        ></input>

                        <select
                            defaultValue="קידומת"
                            onChange={startTelNumOnChange}
                        >
                    ...
                    {telNumErrMsg.length !== 0 && (
                        <span className="input-err-msg">{telNumErrMsg}</span>
                    )}
                ...
                <div className="checkbox-form">
                    ...
                    <span>
                        קראתי ומאשר את <span className="link-span">תקנון</span>{" "}
                        האתר
                    </span>
                </div>
                {checkboxErrMsg.length !== 0 && (
                    <span className="input-err-msg">{checkboxErrMsg}</span>
                )}
            ...
            {isErrNotification && (
                <Notification
                    isSuccess={false}
                    text={"שגיאה כללית אירעה"}
                    setIsNotificationOpen={setIsErrNotification}
                />
            )}
        </>
    );
