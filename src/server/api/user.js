...
export const loginInDB = async (email, password) => {
    try {
        ...
        if (err.response?.status === 400)
            throw new Error("זה לא מה ששמור אצלנו, נסו שנית");
        throw new Error("שגיאה כללית אירעה");
    }
};

export const signupInDB = async (
    email,
    password,
    firstName,
    lastName,
    ...
    try {
        const res = await Axios.post(
            process.env.REACT_APP_ROOT_URL + "signup",
            {
                email,
                password,
                firstName,
        ...
        if (err.response.status === 409) throw new Error(err.response.data);
